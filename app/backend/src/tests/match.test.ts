...
describe('Deve testar os metodos na rota /matches', () => {
  ...
  describe('metodo POST', () => { 
    ...
    it('Deve retornar status 201 e partida criada', async () => {
      ...
      chaiHttpResponse = await chai.request(app).post('/matches').send(match).set("Authorization", response.body.token);
    ...
    it('Deve retornar status 401 e mensagem de erro ao tentar criar partida com dois id de times iguais', async () => {
      ...
      chaiHttpResponse = await chai.request(app).post('/matches').send(sameTeamMatch).set("Authorization", response.body.token);
    
    expect(chaiHttpResponse.status).to.be.eq(401);
    expect(chaiHttpResponse.body.message).to.be.eql("It is not possible to create a match with two equal teams");
    })
    ...
    it('Deve retornar status 401 e messagem de erro ao tentar criar partida com um time inexistente', async () => {
      ...
      chaiHttpResponse = await chai.request(app).post('/matches').send(fakeTeamMatch).set("Authorization", response.body.token);
    
    expect(chaiHttpResponse.status).to.be.eq(404);
    expect(chaiHttpResponse.body.message).to.be.eql("There is no team with such id!");
    })
    ...
    it('Deve retornar status 401 e messagem de erro ao tentar criar partida com token inexistente', async () => {
      ...
      chaiHttpResponse = await chai.request(app).post('/matches').send(fakeTeamMatch).set("Authorization", 'dadasda');
    
    expect(chaiHttpResponse.status).to.be.eq(401);
    expect(chaiHttpResponse.body.message).to.be.eql("Token must be a valid token");
    })
  })
  ...
  describe('metodo PATCH', () => { 
    ...
    it('Deve retornar status 200 ao atualizar gols em uma partida em andamento', async () => {
      ...
      chaiHttpResponse = await chai.request(app).patch('/matches/52').send(updateGoalsMatch);
